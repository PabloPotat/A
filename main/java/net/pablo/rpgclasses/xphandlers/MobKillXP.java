...
public class MobKillXP {
    ...
    public static void onMobDeath(LivingDeathEvent event) {
        ...
        String entityKey = ForgeRegistries.ENTITY_TYPES.getKey(mob.getType()).toString();
        XPUtils.entityDiminish.putIfAbsent(entityKey, 1.0);
        double diminishFactor = XPUtils.entityDiminish.get(entityKey);

        int baseXP = (int) mob.getMaxHealth();
        ...
        for (Map.Entry<UUID, Float> entry : contributions.entrySet()) {
            ...
            int xpGain = (int) (baseXP * (playerDamage / totalDamage) * diminishFactor);
            ...
            contributingPlayer.getCapability(PlayerClassProvider.PLAYER_CLASS_CAPABILITY).ifPresent(cap -> {
                ...
                if (cap.getSecondaryClass() != null) {
                    XPUtils.addXPAndCheckLevel(contributingPlayer, cap,
                            cap.getSecondaryClass().getClassName().toLowerCase(), xpGain / 2);
                }
            });
        }

        // Update diminishing returns
        XPUtils.entityDiminish.put(entityKey, Math.max(0.2, diminishFactor * 0.9));
        XPUtils.lastDecayTime.put(entityKey, System.currentTimeMillis());
